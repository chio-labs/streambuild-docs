...
Each model is one SQL file with a `MODEL()` header and one outer `SELECT`. Models default to
...
|---|---|---|
| `kind` | `table` | `table` for streaming storage or `view` for an ordinary terminal view |
| `relation_name` | none | Exact warehouse relation name, overriding prefix-based naming |
...
| `ttl` | `[defaults].model_ttl` or none | ClickHouse TTL expression |
...
View references do not use `ref_type`: every reference is an ordinary query dependency. Views do
