...
each model name is its SQL filename stem. StreamBuild follows every model's `__source(...)` or
untyped `__ref(...)` driving input transitively until it reaches the source registry. Every pipeline
must resolve to exactly one registered source. Pipelines with no models, multiple source roots,
duplicate model stems, or project-wide duplicate model names are rejected.
...
closure. Pipelines that need no pipeline-wide override omit `pipeline.toml` entirely.
