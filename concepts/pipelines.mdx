...
Both fields are optional. The warning defaults to a generic protected-pipeline message and the
confirmation defaults to the pipeline name, so an empty `[protection]` block is valid.
