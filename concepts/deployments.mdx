...
description: "Opt-in staged builds, assessment, publishing, and lifecycle operations."
---
...
```text
plan -> build -> audit deployment -> publish
```
...
```bash
stb audit deployment --deployment-id 20260729T120000Z_ab12cd
```
...
For `audit deployment` and `publish`, omitting `--deployment-id` works only when exactly one candidate
