...
Selecting a sensor reveals its tick history and any unresolved dead letters — events whose handler
failed every retry, so the sensor's action never ran for them. Retry re-attempts the handler without
repeating completed steps; skip records a reason and drops the event.

<Frame>
  <img src="/images/ui/sensors-detail-dark.png" alt="Expanded sensor showing dead letters with retry and skip actions above the tick history" />
</Frame>
