...
<Note>
  This example enables `[settings] virtual_environments = true` because it demonstrates staged build,
...
and the exact audit and promotion follow-up commands.

List or inspect deployment evidence at any time:

```bash
DEPLOYMENT_ID="replace-with-id-from-build"
stb deployment list --project-dir examples/orders_demo
stb deployment show "$DEPLOYMENT_ID" --project-dir examples/orders_demo
```
...
Stop and investigate if the assessment is `not_ready`; promotion does not enforce that advisory
...
Run live audits after promotion when you want a post-promotion check:
