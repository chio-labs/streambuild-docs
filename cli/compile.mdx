...
StreamBuild builds the static target in staging and atomically replaces `compiled/`,
`manifest.json`, and `streambuild_dag.json`. Runtime artifacts under `target/run/` are preserved.
Sensitive Kafka broker userinfo and source settings are redacted.
