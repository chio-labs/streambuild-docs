...
command atomically writes the same deterministic JSON plan to `target/run/plan/plan.json`.
...
the same JSON through the same serializer. The artifact is disposable visibility output, not local
state, and StreamBuild never reads it to make planning or execution decisions. Failed compilation
or connected planning leaves any previous complete artifact unchanged.
