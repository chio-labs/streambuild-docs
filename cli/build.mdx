...
plan to `target/run/build/deployment-plan.json`. Before any warehouse mutation, it reloads ownership
state and aborts if the confirmed classifications have changed. It never adapts or regenerates the
confirmed plan against the new state. Plan, build, and backfill use separate command-owned paths.
